...
export type State = { version: number; module: number; stage: number; stageMode?: boolean; sims: Record<number, Sim>; autoplay: boolean; nextTickAt: number; running: boolean; elapsed: number; moduleElapsed: number; clockAt: number; pointer: { x: number; y: number; until: number } | null; volume: number; reduced: boolean };
...
const numeric: Record<string, [number, number]> = { options: [2, 6], length: [1, 6], stones: [1, 15], modulus: [3, 16], position: [0, 30], offset: [0, 40], drawers: [2, 8], objects: [1, 16], temperature: [.1, 2], n: [1, 40], capacity: [1, 6] };
...
export function reduce(state: State, command: Command, now = Date.now()): State {
  ...
  switch (command.type) {
    case 'navigate':
    case 'module': { const next = command.type === 'navigate' ? Math.max(0, Math.min(14, s.module + Math.sign(Number(command.value) || 0))) : Number(command.value); if (!Number.isInteger(next) || next < 0 || next > 14) throw Error('Módulo inválido'); s.module = next; if (s.sims[next]) s.sims[next].revealed = false; s.stage = 0; s.autoplay = false; s.moduleElapsed = 0; s.pointer = null; break; }
    case 'advance':
    case 'stage': s.stage = command.type === 'advance' ? Math.max(0, Math.min(7, s.stage + Math.sign(Number(command.value) || 0))) : Math.min(7, Math.max(0, Math.floor(Number(command.value) || 0))); s.autoplay = false; if (s.stage === 5) sim.revealed = true; if (s.stage < 3) sim.revealed = false; break;
    ...
    case 'reset': s.sims[module] = createSim(module); s.autoplay = false; break;
