...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (route.startsWith('/api/')) {
      ...
      if (action === 'notes' && req.method === 'GET') return json(res, 200, getNotes(session.state.module, session.state.stage, Boolean(session.state.mathMode), { quizIndex: session.state.sims[13]?.tick, variant: Number(session.state.sims[session.state.module]?.values.aiVariant || 0), networkQuestion: Number(session.state.sims[14]?.values.networkQuestion || 0) }));
