...
try {
  ...
  for (const session of saved) if (session.expires > Date.now()) { session.state.running = false; session.state.autoplay = false; session.state.clockAt = Date.now(); sessions.set(session.id, session); }
...
setInterval(() => {
  for (const [id, session] of sessions) {
    ...
    if (session.state.running || session.state.autoplay) { session.state = reduce(session.state, { type: 'tick' }); publish(session); }
  }
...
for (const signal of ['SIGINT', 'SIGTERM'] as const) process.on(signal, async () => { for (const session of sessions.values()) { session.state = reduce(session.state, { type: 'clock', value: false }); session.state.autoplay = false; } await persist(); process.exit(0); });
