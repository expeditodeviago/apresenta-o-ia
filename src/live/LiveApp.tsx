...
import { LESSONS, FORMAL_LESSONS, STAGES, STAGE_SECONDS } from '../../shared/curriculum';
...
function PublicStage() {
  ...
  const lesson = state.mathMode && state.module > 0 && state.module < 13 ? FORMAL_LESSONS[state.module] : LESSONS[state.module], sim = currentSim(state);
  ...
  }, [state.module, state.stage, state.autoplay, state.storyPlaying, state.stageMode, aiRoute, send, live.canControl, overview, settings, openPrivate]);
  ...
  return <div className={'live-app ' + (reduced ? 'reduce-motion' : '') + (aiRoute ? ' ai-route' : '') + (state.module === 0 ? ' is-opening' : '') + (state.module === 14 ? ' is-finale' : '') + (state.stageMode ? ' stage-mode' : '')} data-stage={state.stage} data-act={Math.min(3, Math.ceil(state.module / 4))} style={{ '--module-color': state.module === 0 ? '#d6fa87' : lesson.color } as React.CSSProperties} onPointerDown={unlockSound}>
    ...
    <main className="live-main" ref={stageRef}>
      {state.mathMode && AI_CHAPTERS[state.module] && <div className="math-route-banner"><p>{AI_CHAPTERS[state.module].mathBridge}</p><button onClick={() => send({ type: 'mathMode', value: false })}><ArrowLeft size={16} />Voltar à explicação de IA</button></div>}
...
function PrivateController() {
  ...
  }, [live.sessionId, live.canControl, state.module, state.stage, state.mathMode, state.module === 13 ? sim.tick : null, sim.values.aiVariant, sim.values.networkQuestion]);
  ...
  return <div className="control-app" style={{ '--module-color': lesson.color } as React.CSSProperties}><header><div><span className="live-eyebrow">SYNAPSE // APRESENTADOR</span><h1>{lesson.short}</h1></div><span className={'control-connection ' + live.connection}>{live.connection === 'online' ? <Wifi size={17} /> : <WifiOff size={17} />}{live.connection === 'online' ? 'Conectado' : 'Reconectando'}</span></header>
    ...
    </> : <p>Notas indisponíveis. Reconecte o controle para carregá-las.</p>}</div>}
