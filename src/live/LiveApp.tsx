import { activeLesson, isMath, stageTitles } from '../../shared/presentation';
...
import './show.css';
import './journey.css';
import { AIJourney, AIExampleControls } from './AIJourney';
import { AI_CHAPTERS, storyLines } from '../../shared/aiJourney';
import { NOTE_SECTIONS } from '../../shared/noteSections';
...
function PublicStage() {
  ...
  const aiRoute = state.module > 0 && state.module < 13 && !state.mathMode;
  const lesson = activeLesson(state), sim = currentSim(state);
  const titles = stageTitles(state.module, Boolean(state.mathMode)), position = state.module === 13 ? sim.tick : state.stage;
  const changeStage = (value: number, relative = false): Command => ({ type: state.module === 13 ? relative ? 'quizAdvance' : 'quiz' : relative ? 'advance' : 'stage', value });
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowRight' || event.key === 'PageDown') { event.preventDefault(); send(event.shiftKey ? { type: 'navigate', value: 1 } : state.module === 13 ? { type: 'quizAdvance', value: 1 } : { type: 'advance', value: 1 }); }
      if (event.key === 'ArrowLeft' || event.key === 'PageUp') { event.preventDefault(); send(event.shiftKey ? { type: 'navigate', value: -1 } : state.module === 13 ? { type: 'quizAdvance', value: -1 } : { type: 'advance', value: -1 }); }
      if (event.key === ' ') { event.preventDefault(); unlockSound(); send(aiRoute ? { type: 'advance', value: 1 } : { type: 'action', key: 'step' }); }
      if (event.key.toLowerCase() === 'r') send({ type: 'reset' });
      if (event.key.toLowerCase() === 'a') send(aiRoute ? { type: 'storyAutoplay', value: !state.storyPlaying } : { type: 'autoplay', value: !state.autoplay });
      if (event.key.toLowerCase() === 'v' && (isMath(state) || state.module === 13)) send({ type: 'reveal' });
  ...
  return <div className={'live-app ' + (reduced ? 'reduce-motion' : '') + (aiRoute ? ' ai-route' : '') + (state.module === 0 ? ' is-opening' : '') + (state.module === 14 ? ' is-finale' : '') + (state.stageMode ? ' stage-mode' : '')} data-stage={state.stage} data-act={Math.min(3, Math.ceil(state.module / 4))} style={{ '--module-color': state.module === 0 ? '#d6fa87' : lesson.color } as React.CSSProperties} onPointerDown={unlockSound}>
    ...
    <main className="live-main" ref={stageRef}>
      {state.mathMode && AI_CHAPTERS[state.module] && <div className="math-route-banner"><p><strong>Matemática opcional · {lesson.short}</strong><br />{AI_CHAPTERS[state.module].mathBridge}</p><button onClick={() => send({ type: 'mathMode', value: false })}><ArrowLeft size={16} />Voltar à explicação de IA</button></div>}
      {!aiRoute && state.module !== 0 && state.module !== 14 && <>
...
function PrivateController() {
  const live = useLiveSession(true), { state, send } = live, sim = currentSim(state), lesson = activeLesson(state), clock = useClock(state, live.serverOffset);
  ...
  useEffect(() => {
    ...
    setNotes(null); setNotesLoading(true);
    const controller = new AbortController();
    fetch('/api/sessions/' + live.sessionId + '/notes', { signal: controller.signal, headers: { Authorization: 'Bearer ' + credential(live.sessionId) } }).then(async response => { if (!response.ok) { setNotes(null); return; } const data = await response.json(); if (!controller.signal.aborted) setNotes(data); }).catch(() => {}).finally(() => { if (!controller.signal.aborted) setNotesLoading(false); });
    return () => controller.abort();
  }, [live.sessionId, live.canControl, state.module, state.stage, state.mathMode, state.module === 13 ? sim.tick : null, sim.values.aiVariant, sim.values.networkQuestion, state.mathMode ? JSON.stringify(sim) : null]);
  const titles = stageTitles(state.module, Boolean(state.mathMode)), position = state.module === 13 ? sim.tick : state.stage;
  const changeStage = (value: number, relative = false): Command => ({ type: state.module === 13 ? relative ? 'quizAdvance' : 'quiz' : relative ? 'advance' : 'stage', value });
  if (!live.sessionId || !live.canControl) return <div className="private-page"><span className="live-eyebrow">SYNAPSE // CONTROLE PRIVADO</span><h1>Conecte novamente seu controle.</h1>{live.error && <p role="alert">{live.error}</p>}<p>Use o botão abaixo para reconectar à sessão.</p><button className="live-primary" onClick={() => { localStorage.removeItem('synapse-control-' + live.sessionId); sessionStorage.removeItem('synapse-owner-' + live.sessionId); location.reload(); }}>Reconectar controle</button></div>;
  return <div className="control-app" style={{ '--module-color': lesson.color } as React.CSSProperties}><header><div><span className="live-eyebrow">SYNAPSE // APRESENTADOR</span><h1>{lesson.short}</h1></div><span className={'control-connection ' + live.connection}>{live.connection === 'online' ? <Wifi size={17} /> : <WifiOff size={17} />}{live.connection === 'online' ? 'Conectado' : 'Reconectando'}</span></header>
    ...
    <div className="control-stage"><span>{state.module === 13 ? "PERGUNTA" : "ETAPA"} {position + 1} / {titles.length}</span><strong>{titles[position]}</strong><div><button disabled={position === 0} onClick={() => send(changeStage(-1, true))}>Anterior</button><button disabled={position === titles.length - 1} onClick={() => send(changeStage(1, true))}>Próxima etapa<ArrowRight size={16} /></button></div></div>
    ...
    {tab === 'controls' && <><div className="control-transport"><button onClick={() => send(!state.mathMode && AI_CHAPTERS[state.module] ? { type: 'storyAutoplay', value: !state.storyPlaying } : { type: 'autoplay', value: !state.autoplay })}>{state.autoplay || state.storyPlaying ? <Pause size={20} /> : <Play size={20} />}{state.autoplay || state.storyPlaying ? 'Pausar' : 'Auto-Play'}</button><button onClick={() => send(!state.mathMode && AI_CHAPTERS[state.module] ? { type: 'advance', value: 1 } : { type: 'action', key: 'step' })}><ChevronRight size={20} />Um passo</button><button onClick={() => send({ type: 'reset' })}><RotateCcw size={20} />Reiniciar</button>{(isMath(state) || state.module === 13) && <button onClick={() => send({ type: 'reveal' })}><Eye size={20} />{sim.revealed ? 'Ocultar' : 'Revelar'}</button>}</div><>{AI_CHAPTERS[state.module] && <><button className="control-math-toggle" aria-pressed={Boolean(state.mathMode)} onClick={() => send({ type: 'mathMode', value: !state.mathMode })}>{state.mathMode ? 'Voltar à explicação de IA' : 'Explorar a matemática'}</button>{!state.mathMode && <><p className="control-story-summary">{storyLines(state.module, Number(sim.values.aiVariant || 0))[state.stage]}</p><AIExampleControls module={state.module} sim={sim} send={send} /><button className="live-primary" onClick={() => send({ type: 'storyReplay' })}>Repetir animação no projetor</button></>}</>}{state.mathMode || !AI_CHAPTERS[state.module] ? <SimControls module={state.module} sim={sim} send={send} /> : null}</>{state.module === 13 && <SimVisual module={13} sim={sim} send={send} />}<button className="live-primary control-stage-mode" aria-pressed={Boolean(state.stageMode)} onClick={() => send({ type: "stageMode", value: !state.stageMode })}><Maximize size={18} />{state.stageMode ? "Mostrar controles no projetor" : "Modo palco no projetor"}</button><Settings state={state} send={send} /></>}
    {tab === 'notes' && <div className="private-notes">{notes ? <>
      <div className="notes-intro"><strong>{notes.heading}</strong><span>{notes.stage}</span><p>Leia a fala e siga a demonstração durante a apresentação. Abra os tópicos abaixo para estudar o conceito, contar o exemplo completo e responder às dúvidas.</p></div>
      {NOTE_SECTIONS.map(([key, title]) => ['speech', 'demonstration', 'expected'].includes(key) ? <section key={key} data-note={key}><h2>{title}</h2><p>{notes[key]}</p></section> : <details key={key} data-note={key}><summary>{title}</summary><p>{notes[key]}</p></details>)}
      <button className="live-primary" onClick={() => setTab('controls')}>Abrir comandos da demonstração<ArrowRight size={16} /></button>
    </> : <p>{notesLoading ? "Carregando as notas desta etapa…" : "Notas indisponíveis. Reconecte o controle para carregá-las."}</p>}</div>}
