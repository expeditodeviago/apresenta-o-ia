...
import { KnowledgeRemote } from './KnowledgeFinale';
...
export function SimControls({ module, sim, send }: { module: number; sim: Sim; send: (command: Command) => void }) {
  ...
  return <div className="sim-controls">
    {module === 14 && <KnowledgeRemote send={send} sim={sim} />}
    {module === 1 && <>{range('options', 'Opções por posição', 2, 6)}{range('length', 'Comprimento da sequência', 1, 6)}</>}
    {module === 2 && <>{select('algorithm', 'Estratégia de busca', [['bfs', 'Olhar primeiro os lugares próximos'], ['dfs', 'Seguir um corredor até o fim'], ['backtracking', 'Seguir e voltar quando precisar']])}{action('Explorar próximo lugar')}</>}
    ...
    {module === 5 && <>{range('stones', 'Peças disponíveis', 1, 15)}{select('strategy', 'Algoritmo da próxima jogada', [['random', 'Escolher ao acaso'], ['greedy', 'Pegar o máximo agora'], ['minimax', 'Pensar na resposta do adversário']])}{action('Executar estratégia')}<div className="manual-moves">{[1, 2, 3].map(n => <button disabled={sim.done || Number(v.stones) < n} onClick={() => send({ type: 'action', key: 'take', value: n })} key={n}>Retirar {n}</button>)}</div></>}
    {module === 6 && <>{range('modulus', 'Posições no relógio', 3, 16)}{range('position', 'Posição inicial', 0, 30)}{range('offset', 'Deslocamento', 0, 40)}</>}
    {module === 7 && <>{action('Executar próximo grupo de tarefas')}<div className="edge-controls"><span>Dependências diretas</span>{TASKS.flatMap(a => TASKS.filter(b => b !== a).map(b => a + '>' + b)).map(edge => <button key={edge} aria-pressed={sim.edges.includes(edge)} onClick={() => send({ type: 'edge', value: edge })}>{edge.replace('>', ' → ')}</button>)}</div><small>P: pesquisar · V: verificar · E: escrever · R: revisar</small></>}
    {module === 8 && <>{range('drawers', 'Gavetas disponíveis', 2, 8)}{range('objects', 'Objetos para distribuir', 1, 16)}{action('Distribuir automaticamente')}<div className="edge-controls"><span>Colocar próximo objeto</span>{Array.from({ length: Number(v.drawers) }, (_, i) => <button key={i} disabled={sim.done} onClick={() => send({ type: 'action', key: 'place', value: i })}>Gaveta {i}</button>)}</div></>}
    {module === 9 && <>{range('temperature', 'Temperatura', .1, 2, .1)}<small>Menor: concentra nas favoritas. Maior: dá mais espaço às alternativas.</small>{action('Escolher uma continuação')}{action('Repetir 100 escolhas', 'sample100')}</>}
