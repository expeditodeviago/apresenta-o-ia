...
const LEGO_COLORS = [
  { bg: '#38bdf8', text: '#082f49', border: '#0284c7', stud: '#7dd3fc', name: 'Azul Elétrico' },
  { bg: '#f43f5e', text: '#4c0519', border: '#e11d48', stud: '#fb7185', name: 'Coral' },
  { bg: '#f59e0b', text: '#451a03', border: '#d97706', stud: '#fbbf24', name: 'Dourado' },
  { bg: '#10b981', text: '#022c22', border: '#059669', stud: '#34d399', name: 'Verde Menta' },
  { bg: '#6366f1', text: '#1e1b4b', border: '#4f46e5', stud: '#818cf8', name: 'Royal Indigo' },
];
...
export const Act02_LegoTokens: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsSlicing(true);
    setTimeout(() => setIsSlicing(false), 240);
  };
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between p-6 md:p-10 max-w-7xl mx-auto select-none">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold tracking-wide bg-sky-500/10 text-sky-400 border border-sky-500/20">
              Ato 02 • Tokenização & Fatiamento
            </span>
            <span className="text-xs text-slate-400 flex items-center gap-1">
              <Cpu className="w-3.5 h-3.5 text-sky-400" /> BPE (Byte-Pair Encoding)
            </span>
          </div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-white flex items-center gap-3">
            A Máquina de Peças de LEGO
            <span className="text-sm font-normal text-slate-400 hidden sm:inline">
              — Como as LLMs fatiam a linguagem humana
            </span>
          </h2>
          <p className="text-sm text-slate-400 mt-1 max-w-2xl">
            Uma IA não processa caracteres soltos: ela usa uma esteira industrial de alta velocidade para fatiar o texto em <strong className="text-slate-200">Tokens atômicos</strong> e atribuir um código de barras numérico único a cada pedaço.
          </p>
        </div>

        {/* Quick Presets */}
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs text-slate-400 font-mono">Testar:</span>
          {PRESET_PHRASES.map((phrase, i) => (
            <button
              key={i}
              onClick={() => setInputText(phrase)}
              className="text-xs px-2.5 py-1 rounded-lg bg-slate-900/80 hover:bg-slate-800 border border-slate-700/60 text-slate-300 transition-colors"
            >
              {phrase.slice(0, 18)}...
            </button>
          ))}
      ...
      <div className="my-5 relative">
        <div className="relative flex items-center">
          <input
            ...
            placeholder="Digite qualquer frase para fatiar em blocos de Lego ao vivo..."
            className="w-full bg-[#0b0f19] border border-slate-700/70 focus:border-sky-500/80 rounded-xl px-5 py-3.5 text-base md:text-lg text-slate-100 placeholder:text-slate-600 focus:outline-none transition-all shadow-inner font-sans"
          />
          <div className="absolute right-3 flex items-center gap-2">
            <span className="text-xs font-mono text-slate-400 bg-slate-800/80 px-2.5 py-1 rounded-md border border-slate-700/40">
      ...
      <div className="relative flex-1 min-h-[220px] bg-[#0a0d16] border border-slate-800 rounded-2xl p-6 overflow-hidden flex flex-col justify-center shadow-2xl">
        ...
        {/* Conveyor Tracks */}
        <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-b border-slate-950" />
        <div className="absolute bottom-0 left-0 right-0 h-2 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-t border-slate-950" />
        ...
        <div className="absolute top-2 bottom-2 left-10 w-0.5 bg-sky-500/40 border-r border-dashed border-sky-400/60 pointer-events-none flex flex-col justify-between items-center py-2">
          <div className="bg-sky-500 text-slate-950 p-1 rounded-full shadow-lg -translate-x-1/2">
          ...
          <AnimatePresence mode="popLayout">
            {tokens.length === 0 ? (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-slate-500 italic text-sm text-center w-full py-8 font-mono"
              >
                A esteira industrial está vazia. Digite algo acima para iniciar a produção de blocos.
              </motion.div>
            ) : (
              tokens.map((tok, idx) => {
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center min-w-[54px] pt-1">
                        ...
                        <span className="text-[10px] font-mono mt-1 px-1.5 py-0.5 rounded bg-black/25 text-white font-medium tracking-tighter">
                          #{tok.tokenId}
                        </span>
                      </div>
                    </div>
      ...
      {/* Leque de Probabilidade */}
      <div className="mt-5 obsidian-glass rounded-xl p-4 md:p-5 border border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-amber-400" />
            <h3 className="text-xs md:text-sm font-semibold text-slate-200 uppercase tracking-wider">
              Medidor Preditivo: Qual a próxima peça mais provável de se encaixar?
            </h3>
          </div>
          <span className="text-xs text-slate-400 font-mono">
            Distribuição Softmax P(w_t | contexto)
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {predictions.map((item, idx) => (
            <div
              key={idx}
              className="bg-[#0b0f19] border border-slate-800 hover:border-slate-700 rounded-lg p-3 transition-colors flex flex-col justify-between"
            >
